...
) const
{
    ...
    if (this->mesh().moving())
    {
        ...
        scalarField VV0 = this->mesh().V() + this->mesh().V0();
        scalarField V0V00 = this->mesh().V0() + this->mesh().V00();
...
) const
{
    ...
    {
        fvm.diag() = (coefft*rDeltaT2)*this->mesh().V();

        fvm.source() = rDeltaT2*this->mesh().V()*
        (
